import Registry from './registry'
import Adapter from './adapters/json-api/adapter'
import Factory from './factory'

class RestModel {
  #registry = new Registry()
  #factory = new Factory()

  construct() {
    this.#registry.setFactory(
      this.#factory
    )
  }

  setBaseUrl (url) {
    this.#factory.setBaseUrl(url)

    return this
  }

  setHttp (http) {
    this.#factory.setHttp(http)

    return this
  }

  setAdapter (adapter) {
    this.#factory.setAdapter(adapter)

    return this
  }

  setAccessToken (accessToken) {
    this.#adapter.getAdapter().setAccessToken(accessToken)

    return this
  }

  register (name, callback) {
    this.#registry.add(name, callback)

    return this
  }

  resolve (name) {
    return this.#registry.get(name)
  }

  addRequestInterceptor (onFulfilled, onRejected) {
    this.getHttp().interceptors.request.use(onFulfilled, onRejected)
  }

  addResponseInterceptor (onFulfilled, onRejected) {
    this.getHttp().response.use(onFulfilled, onRejected)
  }
}

export default RestModel
